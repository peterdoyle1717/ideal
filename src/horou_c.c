/*
 * horou_c.c — ideal horoball packing solver for triangulated spheres.
 *
 * Input:  face lists from stdin, one graph per line: "a,b,c;d,e,f;..."
 *         (1-indexed vertices; use clers_decode.py to convert CLERS strings)
 * Output: NV float64 per net to stdout (binary):
 *           record[0]   = NaN      (vertex 1 = point at infinity)
 *           record[v-1] = u[v]     v=2..NV  (1.0 for boundary, solved for interior)
 * Record size = NV * 4 bytes; NV is the same for every line in a given prime/N.txt.
 *
 * Compile:  cc -O3 -o horou_c horou_c.c -lm
 * Run:      python3 ../clers/python/clers_decode.py < prime/60.txt | ./horou_c > horou/60.bin
 */
...
#define MAXRING 10          /* max vertex degree (6-nets: ≤6, with headroom) */
#define MAXLINE 16384       /* max input line length */
...
/* ── face list parser ────────────────────────────────────────────────────── */
/* Input: "a,b,c;d,e,f;..."  (1-indexed vertices, semicolon-separated faces) */
...
/* ── horoball geometry ───────────────────────────────────────────────────── */
/* Edge lengths: ui*uj, ui*uk, uj*uk.  Angle at ui-vertex. */
...
/*
 * Solve for horoball weights.
 * v0=1 (point at infinity); its neighbors → boundary (u=1 fixed).
 * Interior vertices solved so angle sum = TAU - defect.
 * Result: u[0]=NaN, u[bndry-1]=1.0, u[interior-1]=solved.
 */
static void horou(double defect, double u_out[]) {
    ...
    for (int iter = 0; iter < 200; iter++) {
        ...
        if (res < 1e-10) { g_iters += iter; break; }
...
int main(void) {
    ...
    while (fgets(line, sizeof(line), stdin)) {
        ...
        fwrite(out, sizeof(double), NV, stdout);
        nets++;
    }
    fprintf(stderr, "nets=%ld  total_iters=%ld  avg=%.2f\n",
            nets, g_iters, nets ? (double)g_iters/nets : 0.0);
    return 0;
}
