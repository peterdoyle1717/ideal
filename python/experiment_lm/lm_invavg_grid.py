#!/usr/bin/env python3
"""LM at α=1° from per-edge 'avg-of-2π/deg' start across small primes.

Start rule: bend(i,j) = mean(2π/deg(i), 2π/deg(j))
            = π · (1/deg(i) + 1/deg(j))

This averages the per-vertex ideal bend, vs the prior `avgdeg` rule
which averages the degree first then divides 2π by it.
...
def main():
    ...
    print(f"alpha={alpha_deg}°  start=invavg(mean(2π/deg(i), 2π/deg(j)) per edge)  v={v_min}..{v_max}")
