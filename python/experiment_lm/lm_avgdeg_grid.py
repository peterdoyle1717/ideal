#!/usr/bin/env python3
"""LM at α=1° from a per-edge 'avg-degree' start across small primes.

Start rule: bend(i,j) = 2π / mean(deg(i), deg(j))
(deg-4-deg-5 edge → 2π/4.5 ≈ 80°; deg-3-deg-3 edge → 120°.)
...
def avgdeg_start(tri, deg):
    return {(i, j): 2.0 * math.pi / (0.5 * (deg[i] + deg[j]))
...
def main():
    ...
    print(f"alpha={alpha_deg}°  start=avgdeg(2π/mean(deg(i),deg(j)) per edge)  v={v_min}..{v_max}")
